...
public enum Pipeline {
    ...
    /// and components smaller than minSpeck pixels (dust). Pages that are
    /// already cropped to the paper (existing PDFs, not a scanner bed) can
    /// pass removeBorder: false so edge-clipped content survives.
    public static func cleanComponents(
        _ bw: inout BinaryImage, minSpeck: Int = 4,
        removeBorder: Bool = true
    ) {
        ...
        for i in 0..<(w * h) where bw.ink[i] {
            let l = Int(labels[i])
            if (removeBorder && touchesBorder[l]) || sizes[l] < Int32(minSpeck) {
                bw.ink[i] = false
            }
        }
    }
        ...
        -> ProcessedPage
    {
        ...
        let c =
            ...
            ? detected
            : Crop(x0: 0, y0: 0, x1: bw.width, y1: bw.height)
        return pack(bw, crop: c, dpi: dpi)
    }

    /// Pack the cropped region of a binary image into a 1-bit page
    /// (MSB first, 1 = white).
    public static func pack(_ bw: BinaryImage, crop c: Crop, dpi: Int) -> ProcessedPage {
