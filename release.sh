#!/bin/zsh
# Build, sign, and package PaperPress as a compressed DMG.
...
# (Same profile PaperDrop uses — one Apple account, one stored credential.)
set -e
cd "$(dirname "$0")"
./bundle.sh

DMG=PaperPress.dmg
scripts/make-dmg.sh PaperPress.app $DMG
